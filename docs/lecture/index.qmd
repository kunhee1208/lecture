...
6)  최대가능도 추정량(Maximum Likelihood Estimator, MLE)
...
-   강의 말미에 등장하는 **최대 가능도 추정량(Maximum Likelihood Estimator)**은 통계적 추론에서 가장 중요한 개념 중 하나이므로 이또한 반드시 이해하고 넘어가자.
...
-   동전을 10번 던져서 나올 수 있는 앞면의 횟수는 0\~10번이고 각 확률은 
$p = _{10}C_{n}\frac{1}{2}^n\frac{1}{2}^{10-n} = _{10}C_{n}\frac{1}{2}^{10}=\frac{_{10}C_{n}}{1024}$로 구할 수 있다.
...
g2=ggplot(f2,aes(n,p))+geom_bar(stat = "identity",width=0.7)+xlab("앞면 횟수")+ylab("확률")+scale_x_continuous(breaks=0:10)+geom_text(data = NULL, x = c(0:10), y = 0.025, label = c("0.001", "0.01", "0.044", "0.117", "0.205", "0.246", "0.205", "0.117", "0.044", "0.01", "0.001"),size=6, colour='burlywood4')+theme(axis.text.x = element_text(size=20,face='bold'))
grid.arrange(g2)
```
...
- **가능도** : 각 사건의 가능도가  `r round(dnorm(1),2)`, `r round(dnorm(0),2)`, `r round(dnorm(1),2)`이므로
`r round(dnorm(1),2)` $\times$ `r round(dnorm(0),2)` $\times$ `r round(dnorm(1),2)` $=$ `r round(dnorm(0)*dnorm(1)^2,2)`
- 연속사건에서 확률과 가능도는 다른 값임을 확인할 수 있다.



## 6. 최대가능도 추정량(Maximum Likelihood Estimator)(1)
...
## 6. 최대가능도 추정량(Maximum Likelihood Estimator)(2)
...
- $(178-\mu)^2+(179-\mu)^2+(180-\mu)^2+(181-\mu)^2+(182-\mu)^2$이 최소가 되도록 $\mu=180$이 되다. 
...
-   본 챕터에서는 통계학에서 필수적인 개념인 **가능도**와 **MLE**를 이해하기 위해 확률과의 비교를 통해 가능도와 MLE의 개념을 순서대로 설명하였으며, 각 순서마다 이산사건과 연속사건의 2가지 예를 제시하였다.
